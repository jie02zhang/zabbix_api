...
def main():
    ...
    if args.host_name and args.file_path:
    ...
    if args.file_path:
        trigger_info = process_hosts_from_excel(args.file_path, zabbix_api, args.trigger_name)
    elif args.host_name:
        host_id, host_name_resolved, host_ip = get_host_id_by_name(zabbix_api, args.host_name)
        if host_id:
            trigger_info = get_trigger_info(zabbix_api, host_id, host_name_resolved, host_ip, args.trigger_name)
        else:
            trigger_info = []
    else:
    ...
    if trigger_info:
        ...
        if args.trigger_status is not None:
            for info in trigger_info:
                trigger_id = info.get("Trigger ID")
                if trigger_id:
                    update_trigger_status(zabbix_api, trigger_id, args.trigger_status)
    else:
...
# **禁用触发器**：
# ```bash
# python c://software/zabbix_api/export_trigger_tags.py --host-name "10.93.203.58" --trigger-name "Ping 连续三次不通" --trigger-status 1
# ```

# **启用触发器**：
# ```bash
# python c://software/zabbix_api/export_trigger_tags.py --host-name "10.93.203.58" --trigger-name "Ping 连续三次不通" --trigger-status 0
# ```